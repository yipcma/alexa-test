...
const getIntentName = (alexaPayload) => {
  return alexaPayload && alexaPayload.request && alexaPayload.request.type === 'IntentRequest' && alexaPayload.request.intent && alexaPayload.request.intent.name
}

const app = botBuilder((message, originalRequest) => {
  ...
  if (message.text) {
    ...
    return 'Spelling bee says ' + message.text + ' is spelled ' + message
      .text
      .toUpperCase()
      .split('')
      .join('. ') + '.';
